...
const verifyEmail = asyncHandler(async (req, res) => {
    ...
    if(!user){
        throw new ApiError(400, "Either Token invalid or expired...")
    }
    // cleanup
...
const resendEmailVerification = asyncHandler(async (req, res) => {
    ...
    await sendEmail({
        ...
        mailgenContent: emailVerificationMailgenContent(
            user.username,
            `${req.protocol}://${req.get("host")}/api/v1/users/verifyemail/${unHashedToken}`
        )
    });
    ...
    return res
            ...
            .json(
                new ApiResponse(
                    200, {}, "Email verification sent to your email ID... ✅"
                )
            )
...
const resetForgotPassword = asyncHandler(async (req, res) => {
    ...
    let hashedToken = crypt0.createHash('sha256').update(resetToken).digest("hex")
...
const changeCurrentPassword = asyncHandler(async (req, res) => {
    ...
    const isPasswordValid = await user.isPaasswordCorrect(oldPassword)
